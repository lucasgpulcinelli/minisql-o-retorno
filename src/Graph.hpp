...
template <class Node, class Edge> class Graph {
    ...
    friend std::ostream& operator<< <Node, Edge>(
        std::ostream& os, const Graph<Node, Edge>& g);

private:
    /*
     * insertAdjacency inserts the Edge instance 'new_edge' in the adjancency
     * list of the Node with address new_edge.idFrom(). If the node does not
     * exist, it inserts an empty node, i.e., inserts an adjacency list with key
     * idFrom(), but does not insert a new node in the map of nodes.
     *
     ...
     * search in the adjacency list, which inserts the Edge if no equivalent
     * edge is found (equivalence as defined by operator==(Edge, Edge)), but
